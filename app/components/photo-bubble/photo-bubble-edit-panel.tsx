...
import { Move, Plus, Upload, X } from "lucide-react";
...
export function PhotoBubbleEditPanel({
	...
	const [selectedBubble, setSelectedBubble] = useState<PhotoBubbleData | null>(
		null,
	);
		...
		try {
			...
			if (selectedBubble) {
				...
				const response = await fetch(
					`/api/photo-bubbles?id=${selectedBubble.id}`,
					{
						method: "PUT",
						headers: {
				...
				if (!response.ok) {
					const errorData = await response.json();
					throw new Error(
						errorData.error || "フォトバブルの更新に失敗しました",
					);
				}
				...
				if (!response.ok) {
					const errorData = await response.json();
					throw new Error(
						errorData.error || "フォトバブルの作成に失敗しました",
					);
				}
						...
						<CardTitle className="flex items-center justify-between">
							...
							{clickedPosition && (
								<span className="text-sm text-gray-500">
									クリック位置: ({Math.round(clickedPosition.x)},{" "}
									{Math.round(clickedPosition.y)})
								</span>
							)}
