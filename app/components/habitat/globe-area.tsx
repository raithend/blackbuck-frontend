...
const GlobeArea = React.memo<GlobeAreaProps>(
	({ customGeographicFile, eraGroups, creator }) => {
		...
		const textureKey = useMemo(() => {
			...
			const dataKey = eraGroups
				.flatMap(
					(eraGroup) =>
						eraGroup.elements?.map(
							(item) =>
		...
		useEffect(() => {
			...
			// 時代グループから生息地データを平坦化
			const dataToUse: HabitatData[] =
				shouldShowHabitat && eraGroups
					? eraGroups.flatMap((eraGroup) => {
