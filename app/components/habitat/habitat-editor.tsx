...
const FabricHabitatEditor = forwardRef(function FabricHabitatEditor(
	...
	const flatHabitatData = useMemo(() => {
		if (Array.isArray(habitatData) && habitatData.length > 0) {
			...
			if ("era" in habitatData[0] && "elements" in habitatData[0]) {
				return (habitatData as EraGroup[]).flatMap(
	...
	const getCurrentGeologicalAgeInfo = useCallback(() => {
		...
		console.log(
			"getCurrentGeologicalAgeInfo呼び出し - selectedMap:",
			selectedMap,
			"selectedAgeIds:",
			selectedAgeIds,
		);
		...
		if (selectedAgeIds.length > 0) {
			...
			if (selectedMap) {
				for (const era of geologicalAgesData.eras) {
					// eraレベルで一致（最も上位の階層）
					if (
						selectedAgeIds.includes(Number.parseInt(era.id)) &&
						era.map === selectedMap
					) {
					...
					for (const period of era.periods) {
						// periodレベルで一致
						if (
							selectedAgeIds.includes(Number.parseInt(period.id)) &&
							period.map === selectedMap
						) {
							console.log(
								"periodレベルで一致（地図確認済み）:",
								era.name,
						...
						for (const epoch of period.epochs) {
							// epochレベルで一致
							if (
								selectedAgeIds.includes(Number.parseInt(epoch.id)) &&
								epoch.map === selectedMap
							) {
								console.log(
									"epochレベルで一致（地図確認済み）:",
									era.name,
							...
							if (epoch.ages) {
								for (const age of epoch.ages) {
									if (
										selectedAgeIds.includes(Number.parseInt(age.id)) &&
										age.map === selectedMap
									) {
										console.log(
											"ageレベルで一致（地図確認済み）:",
	...
	const handleSave = () => {
		...
		console.log(
			"habitatElementsRef.current.length:",
			habitatElementsRef.current.length,
		);

		// localStorageからも復元を試みる
		let currentHabitatElements =
			habitatElementsRef.current.length > 0
				? habitatElementsRef.current
				: habitatElements;
		console.log(
			"保存時の復元処理開始 - currentHabitatElements:",
			currentHabitatElements,
		);
		...
		if (currentHabitatElements.length === 0) {
			try {
				...
				if (saved) {
					...
					if (parsed && Array.isArray(parsed) && parsed.length > 0) {
						...
						console.log(
							"localStorageから復元成功 - currentHabitatElements:",
							currentHabitatElements,
						);
					} else {
			...
			console.log(
				"habitatElementsRefまたはhabitatElementsにデータが存在するため、localStorageからの復元をスキップ",
			);
		}
	...
	const handleSelection = (e: any) => {
		const selected =
			(e.selected?.[0] as FabricObject) || (e.target as FabricObject);
		...
		if (selected) {
			...
			if (obj.habitatPointId) {
				const point = habitatElementsRef.current.find(
					(p) => p.id === obj.habitatPointId,
				);
				if (point) {
	...
	// 生息地ポイントの位置を更新
	const updateHabitatPointPositionHandler = (
		id: string,
		left: number,
		top: number,
	) => {
		console.log("updateHabitatPointPositionHandler呼び出し:", {
			id,
			left,
			top,
		});
	...
	// 生息地ポイントを削除
	const removeHabitatPointHandler = useCallback(
		(id: string) => {
			// Canvasからオブジェクトを削除
			...
			if (
				selectedObject &&
				(selectedObject as FabricObjectWithHabitatId).habitatPointId === id
			) {
				setSelectedObject(null);
			}
	...
	const handleCanvasClick = useCallback(
		(e: any) => {
			...
			console.log(
				"新しいポイントをhabitatElementsに追加後（非同期のため即座には反映されない）",
			);
	...
	const handlePointSelect = useCallback((pointId: string) => {
		if (fabricCanvasRef.current) {
			const obj = fabricCanvasRef.current
				...
				.find(
					(o: FabricObject) =>
						(o as FabricObjectWithHabitatId).habitatPointId === pointId,
				);
			if (obj) {
	...
	const handlePropertyChange = (
		...
		if (
			typeof field === "string" &&
			habitatElementsRef.current.length > 0 &&
			field in habitatElementsRef.current[0]
		) {
			updateSelectedPoint(
				habitatElementsRef.current,
				setHabitatElements,
				field as keyof HabitatElement,
				value,
			);
		}
	};
	...
	useLayoutEffect(() => {
		...
		try {
			...
			console.log(
				"Fabric.jsキャンバスの初期化が完了しました - isInitializedをtrueに設定",
			);
		} catch (error) {
	...
	useEffect(() => {
		console.log(
			"地図画像読み込みuseEffect開始 - fabricCanvasRef:",
		...
		const loadTimeout = setTimeout(() => {
			...
			FabricImage.fromURL(`/PALEOMAP_PaleoAtlas_Rasters_v3/${currentMap}`, {
				...
				.then((img) => {
					...
					setIsLoading(false);
					console.log(
						"地図画像の読み込みが完了しました - isLoadingをfalseに設定",
					);
				})
				.catch((error) => {
	...
	useEffect(() => {
		if (fabricCanvasRef.current && isInitialized && !isLoading) {
			...
			const nonImageObjects = objects.filter(
	...
	useEffect(() => {
		...
		if (flatHabitatData.length > 0 && habitatElements.length === 0) {
			console.log(
				"初期化時: flatHabitatDataをhabitatElementsに設定:",
				flatHabitatData,
			);
	...
	useEffect(() => {
		...
		if (habitatElements.length > 0) {
			...
			localStorage.removeItem("habitatElements");
			console.log(
				"habitatElementsが空になったため、localStorageをクリアしました",
			);
		}
	...
	const pointsTabContent = useMemo(() => {
		console.log(
			"pointsTabContent再計算 - habitatElements:",
			habitatElements.length,
		);
		return (
			<HabitatPointList
		...
		getSelectedObjectId,
		handlePointSelect,
		removeHabitatPointHandler,
	]);

	const propertiesTabContent = useMemo(() => {
		console.log(
			"propertiesTabContent再計算 - habitatElements:",
			habitatElements.length,
		);
		return (
			...
			<div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
				...
				<div className="lg:col-span-1">
					<Tabs defaultValue="age" className="w-full">
						...
						<TabsContent value="age">{ageTabContent}</TabsContent>

						<TabsContent value="points" className="space-y-4">
							{pointsTabContent}
						</TabsContent>

						<TabsContent value="properties">{propertiesTabContent}</TabsContent>
					</Tabs>
				</div>
			</div>
		</div>
