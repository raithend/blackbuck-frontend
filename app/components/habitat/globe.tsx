...
const GlobeComponent = React.memo<GlobeProps>(
	({ customTexture, habitatPoints = [] }) => {
		...
		const [customMapTexture, setCustomMapTexture] = useState<
			string | undefined
		>(undefined);
		const [currentTextureKey, setCurrentTextureKey] = useState<
			string | undefined
		>(undefined);
					...
					if (textureUrl.startsWith("data:image/")) {
						...
						img.onload = () => {
							...
							if (ctx) {
								...
								if (globeRef.current) {
									(globeRef.current.material as THREE.MeshPhongMaterial).map =
										texture;
									(
										globeRef.current.material as THREE.MeshPhongMaterial
									).needsUpdate = true;
								}
							}
						};
						img.onerror = (error) => {
							const truncatedTextureUrl = textureUrl
								? `${textureUrl.substring(0, 20)}...`
								: textureUrl;
							const errorMessage =
